...
description: "Look up MCP LSAM status messages forwarded to the SAM and supporting services to diagnose job and LSAM behavior in Enterprise Manager Operation."
tags:
  - Reference
  - Operations Staff
  - Agents
...
This page is a comprehensive alphabetical reference of all status messages the MCP Agent sends to the SAM and supporting services, including the conditions that trigger each message and the recommended actions for error states. Each entry explains the meaning of the 20-character status description field and the five-character mix number prefix.

- Use this page as the first resource when an unexpected job status appears in Enterprise Manager Operation, since most commonly encountered statuses—such as DS, FAILED-ADOPT, JOB ARRAY FULL, and WFL SYNTAX ERR—are documented here with probable causes and resolution steps.
- Use this page alongside the Processing Variables configuration to understand how task-level checking and Fail Code settings influence which messages are generated for a given job outcome.

The list presents agent status messages forwarded to the SAM and supporting services. The message descriptions are displayed in the graphical interfaces following the job status.
...
All TX messages referenced below indicate a message sent from SMANetCom to the agent.

## ADOPTED

This message indicates that a tracking request has been received by the agent.
...
* In the process of property resolution within OpCon, the data sent to the agent is longer than the allowed maximum length.
...
* An OpCon task initiation request (TX1) was received by the agent, but the initiation attempt failed because of an invalid syntax used in the construction of the file title.
    * JJJJJ is the job number.
    * TTTTT is the task number.
...
* The message occurs when the number of active jobs, according to the OS handler (SMA/MCP/INTERFACE), has reached the maximum defined in the agent configuration file.
...
* This can be the result of any number of situations (multiple SAMs connected to a single agent instance, delayed delivery of a TX1 for a job on which a JOB:TRACK event was sent to SAM, etc.).
...
* The maximum number is defined in the agent configuration file.
* OpCon attempts job initiation again.

## LSAMRESTARTING

* This message is sent following a restart/recovery of the agent modules.
* The job was present in the agent arrays before the agent terminated.
* This is the initial status following an agent recovery.
...
* The mix number of this status description belongs to the agent task that initiates the WFL compile. It is not the mix number for the WFL compile itself.
...
* This message may be received following a restart/recovery of the agent modules.
...
* An OpCon job initiation request (TX1) was received by the agent, but the initiation attempt failed because of a parameter mismatch or invalid attribute (job name does not conform to the rules for entity names).
* If the START command was used, the mix number of this status description belongs to the agent task initiating the WFL compile and not to the mix number for the WFL compile itself. A possible cause for this error is that the Details screen in the Enterprise Manager contains the START or RUN command, but the file named in the File Title is not present. Verify the USERCODE, FAMILY, and spelling.
...
* Following a restart of the agent, the job's status could not be determined due to a GETSTATUS hard error "nnn".
* A description of the specific hard error may be found in the appropriate appendix of the MCP GETSTATUS/SETSTATUS Manual.

## RESTART S_ER nnn

* Following a restart of the agent, the job's status could not be determined due to a GETSTATUS soft error "nnn".
...
An OpCon job initiation request (TX1) was received by the agent, but the initiation attempt failed because of an invalid syntax used in the construction of the file title.
...
* The agent was unable to completely assume the identity (i.e., security privileges, family assignments, and so forth) of the usercode specified on the Details screen in the Enterprise Manager.
* Verify the spelling.
