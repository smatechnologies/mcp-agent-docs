...
description: "Troubleshooting guidance for MCP Agent communication issues including jobs stuck in Running state, LSAM not communicating with SMANetCom, and arithmetic overflow errors."
tags:
  - Reference
...
Multiple SMANetCom components are communicating with a single instance of the MCP Agent. This situation can result in one SMANetCom "stealing" messages intended for the other SMANetCom; as a result, the correct database is not updated with the current job status or external event.
...
The machine name matches neither the BNA host name nor the alias defined in the agent's configuration file.
...
The agent is not configured to accept messages from the SAM machine's IP address. The message "```$CONSOLE:DISPLAY, Bad IP:<Invalid IP Address>```" in the SMANetComTrace log confirms this cause.

##### Fourth Operator Response:

Update the agent configuration to permit communication with the desired IP address. For more information on this response, refer to Communication Parameters (COMM).

#### Fifth Possible Response:
 
The agent detected that the message CRC does not match the calculated CRC value. The message "```$CONSOLE:DISPLAY, Bad CRC:<Partial SMANetCom Message>```" in the SMANetComTrace log confirms this cause.

##### Fifth Operator Response:

An invalid CRC indicates either a problem in the underlying TCP/IP protocol or a bug in the CRC calculation routine by the agent or by SMANetCom. Gather debug information (e.g., the debug print file produced by the \*SMA/TCPIP module, and the SMASUP information from the SAM machine) and contact SMA Technologies Support.
...
2. Set the MCP Agent configuration variable "Task Completion Messages" to "N" to suppress task completion messages from being generated for subordinate tasks. This does not affect reporting of the completion status of the OpCon job.

3. Set the MCP Agent configuration variable "Enable Statistics" to "N". This disables all statistics reporting for all jobs.