...
If an issue occurs, first check [Machine Messages](reference-information/machine-messages.md) and [Troubleshooting](reference-information/troubleshooting/). Most issues that have occurred at other customer sites are documented in these sections. It may be possible to take the prescribed action and be up and running again in a very short time.
...
To view the previous information, refer to [Enable Debug](reference-information/legacy.md#enable-debug) in the Legacy Information topic.
