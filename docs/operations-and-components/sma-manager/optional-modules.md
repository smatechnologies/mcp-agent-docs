...
This field determines whether the agent tracks external jobs. The default value is **N**.
...
This field sets the approximate number of seconds the Resource Monitor should wait between performance metrics samples. The default value is **1800** seconds. Valid values are **0** or **30–86400**. A value of **0** turns off system resource monitoring. For example:
...
This field determines whether the agent initiates the *SMA/JORS/xxx program. Valid values are **N** and **Y**. The default value is **N**.

### JORS Port Number

This field specifies the port number used for communicating job output information with the Enterprise Manager. The default value is **3110**; valid range is **1–65535**.
