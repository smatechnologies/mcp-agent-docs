---
title: Error and Informational Messages
description: "Reference list of all error and informational messages generated by SMA Manager screens, with descriptions and required actions."
tags:
  - Reference
...
The Error and Informational Messages reference lists every message the SMA Manager can display, whether it indicates a configuration error or confirms a successful operation. Each entry identifies the message type (E for error, I for informational) and describes the corrective action required.

- Use error messages to diagnose invalid inputs on screens such as Communication Parameters, DISPMENU, FILEMENU, and RESMENU.
- Use informational messages to confirm that operations such as starting or stopping the MCP Agent, reloading configuration files, or saving definitions completed successfully.

The following table lists all messages, their type, and the required action.
...
| Changes saved. REMOVE TRACKING AND CHECKPOINT FILES. | I | The number of maximum jobs has been changed. Remove the \*SMA/TRACKING/FILE/xxx and \*SMA/CP/= files before starting the agent again to avoid a mismatch in file and array sizes. |
|Communication configuration saved | I  | Changes made on the SMACOMM screen have been saved to the agent configuration file. |
...
| LSAM and Resource Monitor submitted for start	| I | The agent WFL is configured to start both the agent and the Resource Monitor, and the WFL was submitted (INITLSAM). Use the STATUS screen to confirm a successful initialization. |
| LSAM submitted for start | I | The necessary fields to start the agent are defined, and the WFL was submitted (INITLSAM). Use the STATUS screen to confirm a successful initialization. |
| LSAM was instructed to load configuration file | I | The agent was instructed to reload the configuration values from the latest configuration file (LOADDISP). All configuration fields except for the Maximum Concurrent Number of Jobs value are refreshed. <br></br><br></br> To refresh the Maximum Concurrent Number of Jobs, complete the following steps: <br></br><br></br> 1. Allow all active jobs to complete <br></br> 2. Stop the agent <br></br> 3. Remove the \*SMA/TRACKING/FILE/xxx and \*SMA/CP/= files <br></br> 4. Start the agent |
| LSAM was instructed to load global displays file | I | The Display Handler was instructed to reload the global displays file, \*SMA/DISPLAYS. |
| LSAM was instructed to start debug (for n minutes) | I | The agent was instructed to start debugging. If a time limit was entered on the INITDEBUG screen, it is also included in the message. |
| LSAM was instructed to stop | I | The agent was instructed to stop (STOPLSAM). |
| LSAM was instructed to stop debug | I | The agent was manually instructed to discontinue debugging (STOPDEBUG). |
...
| No mix entries found for this LSAM instance | I | The STATUS screen was queried and no processes associated with this agent instance were found to be active. |
...
| Resource Monitor is included in LSAM WFL | E | The Resource Monitor is configured to be included within the agent WFL, so it cannot be started separately using INITRM. Use INITLSAM to start both the agent and Resource Monitor. Alternatively, use the General Configuration screen to configure the Resource Monitor to run in its own WFL. |
...
| Resource Monitor was instructed to stop | I | The Resource Monitor was instructed to stop (STOPRM). The Resource Monitor must always be stopped independently of the agent, regardless of how it was initiated. |
...
| This instance of the LSAM is not active | E | A screen was submitted that acts upon the agent, but the agent is not active (INITDEBUG, STATUS, STOPDEBUG, STOPLSAM). |
...
| Too many config files selected | E | Multiple agent configuration backup files were selected for restore. Only one file of each type may be selected. | 
...
| Use GEN option to configure required information | E | The INITLSAM screen was submitted but required data has not been configured on the GEN screen, or the INITRM screen was submitted but required data has not been configured on the GEN screen. |