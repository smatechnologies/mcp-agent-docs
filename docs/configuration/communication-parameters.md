...
- Use this page when initially connecting the MCP Agent to an OpCon server to ensure the port number, hostname, and TLS settings match the corresponding values defined in the Solution Manager machine record.
...
| Hostname alias | This field defines the value that corresponds to the machine name defined in Solution Manager. If left blank, the system hostname will be used. When providing a value for this field, do not include an ending period (.); however, when defining the machine name in Solution Manager, the trailing period (.) must be used in the Machine definition. | 
