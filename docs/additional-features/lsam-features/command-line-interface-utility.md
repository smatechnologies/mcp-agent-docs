...
Because processes initiated via a RUN command are limited to a single parameter when initiated using OpCon or CANDE, it is necessary to initiate the \*SMA/COMMAND program using a WFL. The program may be contained in a user-written WFL or initiated using the SMA/WFL/COMMAND WFL. Refer to [Using *SMA/WFL/COMMAND](../../additional-features/utilities/sma-wfl-command.md).
