...
Modify the following fields under [Optional Modules (OPT)](../../configuration/optional-modules.md):
...
If you suspect that you are experiencing a delay due to high display message volume, you may determine the number of display messages waiting to be processed by selecting the MSGCOUNT choice of the SMAMGR Main Menu. The display message backlog will be returned. To view the previous information, refer to [Create the Necessary Displays File(s)](../../reference-information/legacy.md#create-the-necessary-displays-files) in the Legacy Information topic.
...
To view the previous procedure, refer to [Define an Automated Response](../../reference-information/legacy.md#define-an-automated-response) in the Legacy Information topic.
...
A WFL (JOB1) performs multiple actions and another job (JOB2) is waiting for output from one of the actions in JOB1. Instead of JOB2 waiting until JOB1 finishes completely, the key action in JOB1 can display a unique message after it completes. As a result, the message triggers the SAM to start JOB2.
